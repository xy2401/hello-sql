...
description: 对比 22 个可本地运行数据库产品的容器形态、客户端命令、结构查看、查询闭环和验证等级
---

# Docker、连接 CLI 与查询证据

所有 22 个产品都有条目，只收录可自托管、嵌入式或浏览器本地运行的数据库。状态分为：可由无凭证 CI 实测、受限官方镜像、浏览器环境不适用。
...
| KV/流 | Redis、Valkey | `redis-cli`、`valkey-cli` | 固定数据写入、读取与结构复查 |
